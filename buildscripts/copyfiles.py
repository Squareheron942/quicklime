...
def process(args: argparse.Namespace): 
    ...
    if os.path.isdir(args.assets):
        ...
        for file in files:
            if not file in file_edit_times or file_edit_times[file] < os.path.getmtime(file):
                make_folder_if_not_exist(os.path.join(args.romfs, 'scenes', os.path.basename(file)))
        ...
        subfolders, files = run_fast_scandir(args.assets, mdl_ext) # copy all model files to the corresponding folders
        for file in files:
            if not file in file_edit_times or file_edit_times[file] < os.path.getmtime(file):
                make_folder_if_not_exist(os.path.join(args.romfs, file))
                print(os.path.join(args.romfs, file))
    ...
    if os.path.isdir(args.include):
        ...
        with open(os.path.join(args.include, "shaders.inc"), 'w+') as matheader:
            matheader.write('#pragma once\n')
            matheader.writelines(('#include "' + s + '"\n') for s in materials) 
