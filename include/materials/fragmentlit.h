...
class fragment_lit : public material {
    ...
    fragment_lit(FILE* args) noexcept {
        if (args) {
            ...
            if (!loadTextureFromFile(&tex, NULL, "romfs:/gfx/kitten.t3x", false)) Console::warn("unable to load texture");
            // mat = m_args.mat;
            mat = { // placeholder while i figure out the model converter stuff
