...
class AudioManager {
  ...
  public:
	...
	@return NDSP channel ID the audio is played on, or -1 if no channel available
	*/
	static ndsp_channel requestChannel(channel_prio priority, const char* filename);
	/*
	Stops given channel from playing and ends any thread which is currently using it. 
	Thread may keep playing until the end of the buffer is reached (~120ms max)
	*/
