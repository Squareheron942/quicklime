...
class SceneLoader {
	...
	static void parseComponent(std::unique_ptr<Scene> &s, GameObject &object,
							   std::string_view input);
	static void parseComponents(std::unique_ptr<Scene> &s, GameObject &object,
								std::string_view &input);
  ...
  public:
	...
	/**
	 * @brief Load scene from file. The returned unique_ptr must be kept alive
	 * until progress = 1.
	 *
	 * @param file Path of .scene file to read
	 * @return AsyncSceneLoadOperation Information about scene load progress. If
	 * scene not found, progress will be -1
	 */
