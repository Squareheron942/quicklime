...
#if CONSOLE
	LightLock_Guard l(_l);
	if (controls::getDown(controls::key::KEY_SELECT))
		nextMenu();
	switch (menu) {
	case MENU_STATS:
		...
		printf("\e[s\e[%u;%uHDraw Calls: %u\e[u", DRAW_Y, DRAW_X,
			   stats::_drawcalls);
		printf("\e[s\e[%u;%uHCurrent Scene: %s            \e[u", SCN_Y, SCN_X,
			   SceneManager::currentScene->name.c_str());
		printf("\e[s\e[%u;%uHHeap Usage: %u KiB         \e[u", MEM_Y, MEM_X,
			   mallinfo().uordblks / 1024);
		printf("\e[s\e[%u;%uHLinear RAM Free: %lu KiB         \e[u", MEM_Y + 1,
			   MEM_X, linearSpaceFree() / 1024);
		printf("\e[s\e[%u;%uHVRAM Free: %lu KiB         \e[u", MEM_Y + 2, MEM_X,
			   vramSpaceFree() / 1024);
