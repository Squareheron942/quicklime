...
void AudioSource::update() {
	...
	if (mute) {
		...
		parent->s.reg.view<AudioListener>().each([&](AudioListener &listener) {
			if (listener.active)
				l = listener.parent;
		});
		ASSERT(l != nullptr, "No listener in scene");

		float dist = FVec3_Distance(parent->getComponent<transform>()->position,
...
constexpr float AudioSource::volRollOffLin(const float dist, const float min,
										   const float max) {
