...
namespace ql {
	C3D_RenderTarget *Camera::target[4] = {
		...
		C3D_RenderTargetCreate(
			...
			GPU_RB_DEPTH24_STENCIL8)}; // 2 targets in case of stereoscopic
									   // view, plus one for if wide view is
									   // also enabled (since it's a different
									   // resolution)

	namespace {
		/**
		 * @brief The default used is just iod/5 (probably to reduce just how
		 * much it splits apart which causes headaches). Having a map function
		 * allows the user to basically do whatever they want though. You can
		 * have it completely ignore the input and use your own variable, make
		 * it exponential, log, alternate between 0 and 100 etc
		...
		struct cam_args {
			...
			bool active = true;
			bool wide	= true; // whether or not to use the 240x800 mode on
								// supported models
			bool ortho	= false;
			...
			unsigned short
				resolution; // min 8x8, max 1024x1024 (must be square)
	...
	Camera::Camera(GameObject &parent, const void *args) {
		...
		case RENDER_TYPE_TOPSCREEN:
			...
			highRes = c.wide &&
					  !config::wideIsUnsupported; // disable if not supported
	...
	void Camera::Render() {
		...
		culledList.clear(); // remove all the old objects without deallocating
							// the space since 99% of the time it won't need to
							// change the space
		...
		// Mtx_Multiply(&vp, &projection, &view); // create view projection
		// matrix, much faster since it saves a ton of matrix multiplications
		// and is also useful for frustum culling
		...
		// nearN.z*nearN.z); float invfarM = 1/sqrtf(farN.x*farN.x +
		// farN.y*farN.y + farN.z*farN.z); float invtopM = 1/sqrtf(topN.x*topN.x
		// + topN.y*topN.y + topN.z*topN.z); float invbotM =
		// 1/sqrtf(botN.x*botN.x + botN.y*botN.y + botN.z*botN.z); float
		// invleftM = 1/sqrtf(leftN.x*leftN.x + leftN.y*leftN.y +
		// leftN.z*leftN.z); float invrightM = 1/sqrtf(rightN.x*rightN.x +
		// rightN.y*rightN.y + rightN.z*rightN.z);
		...
		//     // mesh* m = NULL;
		//     // if (!obj) continue; // skip null objects, there shouldn't be
		//     any so I think it can be removed, and I can't remove it from the
		//     list at this step so it might end up being kinda slow Renderer*
		//     r; if (!(r = obj.getComponent<Renderer>())) continue; // skip
		//     objects with no set renderer
		...
		//     // if (!(left + right + back + front + top + bot)) continue; //
		//     if not in any then skip it (this means it is outside the frustum)
		//     // culledList.push_back(r);
		// }
		osTickCounterUpdate(&stats::profiling::cnt_cull);
		stats::profiling::rnd_cull =
			osTickCounterRead(&stats::profiling::cnt_cull);
		// actually render stuff for left eye
		osTickCounterStart(&stats::profiling::cnt_meshrnd);
		// render objects
		parent->s.reg.view<Renderer, transform>().each(
			[&](auto &rnd, auto &trans) {
				...
				// if (!(left + right + back + front + top + bot)) continue; //
				// if not in any then skip it (this means it is outside the
				// frustum)
		...
		// render text
		// C2D_SceneTarget(target[(type == RENDER_TYPE_TOPSCREEN) ? (stereo ? 0
		// : 2) : 3]); //TODO defer text/ui rendering
		...
		C3D_RenderTargetSetOutput(target[1], GFX_TOP, GFX_RIGHT,
								  CAM_DISPLAY_TRANSFER_FLAGS);
		Mtx_PerspStereoTilt(&projection, C3D_AngleFromDegrees(fovY),
							C3D_AspectRatioTop, nearClip, farClip, iod,
							focalLength, false);
		...
		// render text
		// C2D_SceneTarget(target[(type == RENDER_TYPE_TOPSCREEN) ? (stereo ? 1
		// : 2) : 3]); //TODO defer text/ui rendering
	}
