...
transform::transform() : _position({{1, 0, 0, 0}}), _rotation({{1, 0, 0, 0}}), _scale({{1, 1, 1, 1}}) {};
transform::transform(GameObject& parent, const void* data) : _position({{1, 0, 0, 0}}), _rotation({{1, 0, 0, 0}}), _scale({{1, 1, 1, 1}}) {
    if (data) {
...
transform::transform(C3D_FVec position, C3D_FQuat rotation, C3D_FVec scale) : _position(position), _rotation(rotation), _scale(scale) {};
...
void transform::rotateX(float angle, bool local) { LightLock_Guard l(_l); _rotation = Quat_RotateX(rotation, angle, local); }
void transform::rotateY(float angle, bool local) { LightLock_Guard l(_l); _rotation = Quat_RotateY(rotation, angle, local); }
void transform::rotateZ(float angle, bool local) { LightLock_Guard l(_l); _rotation = Quat_RotateZ(rotation, angle, local); }

void transform::setRotation(C3D_FVec attitude, bool local) { LightLock_Guard l(_l); _rotation = Quat_FromPitchYawRoll(attitude.x, attitude.y, attitude.z, local); }
