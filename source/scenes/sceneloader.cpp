...
void sceneLoadThread(void *params) {
	...
	SceneLoader::parseObjectAsync(p.s, text, text.size(),
...
AsyncSceneLoadOperation SceneLoader::loadAsync(std::string name) {
	...
	sceneLoadThreadParams *p = new sceneLoadThreadParams{
		std::make_unique<Scene>(name), name, 0.f, false, true, LightEvent()};
	...
	// Start the thread
	threadCreate(sceneLoadThread, p, SCENELOADER_THREAD_STACK_SZ, priority, -1,
				 true);
...
void SceneLoader::parseChildren(std::unique_ptr<Scene> &s, GameObject &object,
...
void SceneLoader::parseScripts(std::unique_ptr<Scene> &s, GameObject &object,
							   std::string_view &input) {
	while (input[0] != ']') { // ] means end of the section
		if (input.find(',') <
			...
			ComponentManager::addScript(
				std::string(input.substr(0, input.find(']'))).c_str(), object);
...
void SceneLoader::parseComponents(std::unique_ptr<Scene> &s, GameObject &object,
								  std::string_view &input) {
	while (input[0] != ']') { // ] means end of the section
		if (input.find(',') <
			input.find(']')) { // this means there is a , closer than a ] so
...
void SceneLoader::parseComponent(std::unique_ptr<Scene> &s, GameObject &object,
								 std::string_view input) {
	...
	if (input.find('}') - input.find('{') - 1 > 0) {
		base64data =
			base64::from_base64(input.substr(
				input.find('{') + 1, input.find('}') - input.find('{') - 1)) +
			'\0';
