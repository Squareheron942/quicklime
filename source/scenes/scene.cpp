...
void Scene::awake() {
	LightLock_Lock(&lock);
	act_on_objects(
		&GameObject::Awake); // call awake() on every gameobject and enable them
							 // (to self disable do it when this is called)
...
void Scene::update() {
	LightLock_Guard l(lock);
	act_on_objects(&GameObject::Update); // call update() on every gameobject
